...
const getAllShareHolders = new Promise((resolve, reject) => {
    var options = {
        'method': 'GET',
        'url': 'https://api.thamturakit.com/api/withdrawcerts',
        'headers': header
    };
...
getAllShareHolders.then(res => {
    ...
    worksheet.cell(1, 1).string('เลขที่ใบถอนหุ้น')
    worksheet.cell(1, 2).string('วันที่')
    worksheet.cell(1, 3).string('เลขที่ผู้ถือหุ้น')
    worksheet.cell(1, 4).string('ชื่อผู้ถือหุ้น')
    worksheet.cell(1, 5).string('จำนวนหุ้นที่ถอน')
    worksheet.cell(1, 6).string('มูลค่าหุ้นที่ถอน')
    ...
    workbook.write(`รายงานถอนหุ้นตั่งแต่เริ่มกิจการ.xlsx`);
})