...
const getOnlineOrders = new Promise((resolve, reject) => {
    ...
    request(options, function (error, response) {
        ...
        const result = res.data.filter((data) => {
            return data.payments.length > 0 && data.payments[0].billPaymentRef2 === 'SHAREHOLDER' && new Date(data.payments[0].transactionDateandTime) > new Date('2021-11-01') && new Date(data.payments[0].transactionDateandTime) < new Date('2021-12-01');
        })
        resolve(result);
...
getOnlineOrders.then((res) => {
    getAllContacts.then((contacts) => {
        ...
        res.forEach(order => {
            ...
            worksheet.cell(i, 4).number(order.items[0].netAmount)

            i++;
        });

        workbook.write(`รายการรับเงินซื้อหุ้นเดือน11.xlsx`);
    });
})